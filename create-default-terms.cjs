...
async function createDefaultTerms() {
  try {
    ...
    const existingTerms = await db.execute(sql`SELECT * FROM terms_and_conditions LIMIT 1`);
    
    if (existingTerms.rows.length > 0) {
      console.log("Terms and conditions already exist. Skipping creation.");
      console.log("Existing terms count:", existingTerms.rows.length);
      return;
    }
    ...
    const result = await db.execute(sql`
      INSERT INTO terms_and_conditions (title, content, version, is_active, last_modified_by)
      VALUES (
